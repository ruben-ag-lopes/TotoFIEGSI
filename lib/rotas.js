...
const { todasAsJornadas, contarAcertos, apostasFechadas: fechouJornada } = require('../jornadas');
const db = require('./db');
...
const rotas = {
  ...
  'GET /api/jornadas': async (req, res) => {
    ...
    const jornadas = todasAsJornadas().map((j) => {
      ...
      // Os palpites so ficam publicos depois da data limite: ate la ninguem
      // ve (nem copia) a chave dos outros. Por isso nem saem do servidor.
      const classificacaoVisivel = fechouJornada(j);

      const classificacao = !classificacaoVisivel ? [] : apostas
        .map((a) => ({
  ...
  /*
   * Jogo Fantasy — lido da Google Sheet publicada em CSV.
   * A folha é a fonte da verdade; aqui só se lê e se apresenta.
   */
  'GET /api/fantasy': async (req, res) => {
    try {
      const dados = await fantasy.obter();
      json(res, 200, dados);
    } catch (err) {
      console.error('[fantasy]', err.message);
      json(res, 502, { erro: err.message });
    }
  },
